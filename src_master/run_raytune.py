...
def train_sgnn(opt, checkpoint_dir = None):
    ...
    train.report({"val_acc":np.mean(best_val_acc), "test_acc": np.mean(best_test_acc)})
...
if __name__ == '__main__':
    ...
    search_space = {
        ...
        'weight_decay': tune.loguniform(1e-6,5e-2),
        'num_h_layers':tune.choice([0,1,2]),
        'hidden': tune.choice([8,16,32,64]),
        'input_dropout': tune.uniform(0,0.6),
        'dataset':args.dataset,
        'geom_gcn_splits':args.geom_gcn_splits,
