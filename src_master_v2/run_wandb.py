...
import numpy as np
import torch
from la_sgnn import SGNN, SGNN_NoInteraction, SGNN_version2, SGNN_version3, SGNN_Attention, SGNN_source, SGNNLinear
import os
import wandb
...
def train_sgnn():
    ...
    for rep in range(10):
        ...
        data = data.to(device)
        model = SGNNLinear(opt).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dropout', type = float, default=0.5, help='Dropout before output layer.')
    ...
    sweep_id = wandb.sweep(sweep_config, project= args.dataset + '_linear_dropout', entity='alanjohnvarghese')
